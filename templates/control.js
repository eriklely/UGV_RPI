...
function joyStickCtrl(inputX, inputY) {
    if (module_type == 1) {
        ...
        if (steady_mode == false) {
            cmdJsonCmd({"T":cmd_gimbal_ctrl,"X":inputX/2.5,"Y":-inputY/2.5,"SPD":0,"ACC":0});
        } else {
            steadyCtrl(1, inputY);
        }
...
var nerfFlywheelOn = false;
function nerfFlywheelCtrl(enable) {
    ...
    removeButtonsClass(buttons);          // clear previous highlight
    if (enable == 1) {
        buttons[2].classList.add("ctl_btn_active");
    } else {
        buttons[0].classList.add("ctl_btn_active");
    }
}
...
function readGamepad() {
  ...
  for (var i = 0; i < gamepads.length; i++) {
    ...
    if(gp) {
      ...
      if (last_gp_lt1 != gp.buttons[6].pressed) {
          if (gp.buttons[6].pressed) {
              nerfFlywheelCtrl(1);          // Flywheel ON
          } else {
              nerfFlywheelCtrl(0);          // Flywheel OFF
          }
          last_gp_lt1 = gp.buttons[6].pressed;
      }

      if (last_gp_lt2 != gp.buttons[4].pressed) {
          if (gp.buttons[4].pressed) {
              cmdJsonCmd({'T':147});        // Fire one dart
          }
          last_gp_lt2 = gp.buttons[4].pressed;
      }
      ...
      if (module_type != 1) {
          ...
          if(last_gp_pt_x != gp_pt_x || last_gp_pt_y != gp_pt_y){
            cmdJsonCmd({"T":cmd_gimbal_ctrl,"X":gp_pt_x,"Y":gp_pt_y,"SPD":100,"ACC":8});
          }

          var change_x = gp.axes[2];
          if(Math.abs(change_x) < 0.10){
            change_x = 0;
          }
          var change_y = gp.axes[3];
          if(Math.abs(change_y) < 0.10){
            change_y = 0;
          }
